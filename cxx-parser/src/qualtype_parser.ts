...
import {_FlattenNode,dumpClangASTJSON ,filterAndFlattenNodes,ClangASTNodeKind,TagUsedType} from './utils';
...
export class ASTNodeKey {
 ...
 export class ASTNodeMap {
  ...
  get(node: CXXTerraNode): string | undefined {
      ...
      let parent_full_scope_name = node.asVariable()?.parent_full_scope_name;
      let param_name = node.asVariable().name;
...
function _parseQualTypes(
  ...
  function collectQualTypeInfos(node: any, val_parent_full_scope_name: string = '') {
    ...
    let bIsKind_Class = node.kind == ClangASTNodeKind.CXXRecordDecl && node.tagUsed == TagUsedType.class_t;
    let bIsKind_Method = node.kind == ClangASTNodeKind.CXXMethodDecl;

    if (bIsKind_Class || bIsKind_Method) {
      val_parent_full_scope_name = val_parent_full_scope_name ? `${val_parent_full_scope_name}::${node.name}` : node.name;
    }
